...
def upart_ini(box, ee, ti, xi, yi, zi,nx,ny,nz,uij,na):

    for ii in range(na):
        i = mv[ii] # select a movable particle (metal atoms)
        # old attraction
        k = box[(xi[i]+1)%nx,(yi[i]+1)%ny,zi[i]]
        if k >= 0:
            uij[i,k] = ee[ti[i],ti[k]]
            uij[k,i] = uij[i,k]
            if ti[i] == 2 and ti[k] == 3:
                v = np.sum(vij[i,:])
                if v == 0 or v == 0.8:
                    vij[i,k] = 1.2
                    vij[k,i] = vij[i,k]
...
# model
    ...
    ee[1,3] = -10.0 # O - Ca(II)
    ee[3,1] = ee[1,3]
    ee[2,3] = -12.0 # O - La(III)
    ee[3,2] = ee[2,3]

    ee[1,1] = 10.0 # Ca-Ca Coulombic repulsion
    ee[2,2] = 15.0 # La-La Coulombic repulsion
...
# configuration
    ...
    uij = np.zeros((na,na), dtype=float) # bond energy
    vij = np.zeros((na,na), dtype=float) # bond valence (for oxygens)
    ...
    # initial energy
    upart_ini(box, ee, ti, xi, yi, zi, ny,nz,uij,na)
